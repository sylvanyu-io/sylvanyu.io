import { Shader, Engine, Color, Texture2D, RenderQueueType, CullMode, BaseMaterial, Vector4, Vector2 } from '@galacean/engine'
...
Shader.create(
  ...
  void main(){
    ...
    vec3 positionWS = (renderer_ModelMat * vec4(POSITION,1.)).xyz;
    ...
    gl_Position = camera_VPMat*vec4(positionWS,1.);
  ...
  void main(){
    ...
    vec4 posWSFromDepth=camera_ViewInvMat*v_ProjectionInvMat*vec4(posNDC,1.);
    ...
    vec3 viewDir=normalize(camera_Position-v_PosWS);
