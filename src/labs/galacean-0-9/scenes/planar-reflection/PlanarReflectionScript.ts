...
import { Camera, MeshRenderer, Vector3, Script, Texture2D, Vector4, RenderTarget, TextureFormat, Material, Matrix } from '@galacean/engine'
...
export class PlanarReflectionScript extends Script {
  ...
  onAwake() {
    ...
    this._sharedMaterial = this._meshRenderer.getMaterial()
    // @ts-ignore
  ...
  onUpdate(): void {
    ...
    this._reflectionCamera.enabled = true
    this._reflectionCamera.render()
  ...
  private renderReflection(currentCam: Camera, reflectCamera: Camera): void {
    ...
    const normal = reflectiveSurface.transform.worldUp.clone()
